...
class Node:
    ...
    def __init__(self, env, node_id, resource_name, capacity):
        ...
        self.resource = sp.Resource(env,capacity=capacity)
...
class PathwayModel:
    ...
    def patient_journey(self, patient):
        ...
        while go_on:
            ...
            with self.nodes[patient.current_node_id].resource.request() as req:
...
def plot_time_in_system(data, start, finish, analysisID, analysis_location):
    ...
    if len(data)>1:
        for i in range(2,len(data)):
            all_results = all_results.append(data[i].time_in_system)
